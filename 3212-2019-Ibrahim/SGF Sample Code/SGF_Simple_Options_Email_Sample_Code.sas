...
select email.*, cat(strip(employee_data.first_name)," ",strip(employee_data.last_name)) as name, employee_data.first_name, employee_data.last_name, employee_data.department
from employee.email left join employee.employee_data on
email.employee_number=employee_data.employee_number;
run;

/* Replace email recipient with your email address for testing purposes */
data employee.contact;
set employee.contact;
email="YOUREMAILADDRESS";
run;

/* Define macro to send employee compensation reports */
%macro sendreports(department,email,name);
...
data _null_;
  ...
  call execute(cats('%sendreports(',department,',',email,',',name,')'));
  end;
 run; 
